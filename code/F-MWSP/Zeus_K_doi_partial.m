...
%			save_each_itt:  save the entire space  at each itt
%					
%			
%
...
%			index 7: time of index 1 solving master lp.  the rest is junk computation
%			for entire problem.  use 2,5,7.  5 is ilp on last itteration
...
G.B.sub_prob_order=randperm(G.B.num_pricing);
current_index_sub_problem=1;
...
while(true) 
	...
	[G,T,residual,inds_new,t6,current_index_sub_problem]=do_pricing_partial(G,C,T,lambda,psi,pricing_option,current_index_sub_problem);
	t2=toc(t2);
	lb=lp+residual;
	...
	if(isfield(G.opt,'save_each_itt'))
		if(G.opt.save_each_itt>0.5)
			disp('saving')
			save(G.save_running_name)
			disp('done saving')
		end
	end
	step=step+1;
