...
%			min_cost_terminate=if this ub is not achieved then we will not terminate based on the itteration
...
if(~isfield(G.opt,'min_cost_terminate'))
	G.opt.min_cost_terminate=1;
end
...
while(true) 
	...
	if(step>=G.opt.max_steps && ub<G.opt.min_cost_terminate)
