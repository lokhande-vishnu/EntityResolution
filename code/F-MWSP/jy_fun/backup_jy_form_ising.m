...
if(Z.B.N_pos+Z.B.N_neg+numel(hard_exclude_cliques)>0.5)
	%compute max_val	
	max_val_ub=100-2*(   sum(unary(unary<0))+pair(pair<0)  );
	%compute unary terms 
	Z.graph.unary=unary(Z.B.var_keep);
	inds_force_on_red=ismember(   Z.B.inds_force_on, var_keep  );
	...
	if(Z.B.N_pos+Z.B.N_neg)
		...
		pairwise_str(:,[1,2])=[E([pos_inds;neg_inds],:)];%set indexes
	...
	[~,pairwise_str(:,[1:2])]=ismember(   pairwise_str(:,[1:2]), var_keep  );
	%make sparse
	Z.graph.pairwise_str=sparse( pairwise_str);
