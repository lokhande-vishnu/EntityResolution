...
function [sol,cost,red_cost,used_psi]=jy_price_ising(Z,lambda,C_list,psi,inst_cost,force_on,force_off,lb_obj)
%purpose
%
	...
	if(numel(obj)==1)
		...
		if(1==0)
			...
			try
				...
				if(use_heur==0)
					gc_sol = QPBO_wrapper_mex(unary_w_dual', Z.graph.pairwise_str, int32(Z.B.var_keep*0), 'q');
				end
				if(use_heur==1)
					gc_sol = QPBO_wrapper_mex(unary_w_dual', Z.graph.pairwise_str, int32(Z.B.var_keep*0), 'i');
					gc_sol=double(gc_sol);
				end
			catch
				disp('eror in runner for exact pricing pre')
				save('exactPriError')
				pause
			end
			gc_sol=double(gc_sol);
		...
		if( (red_cost+inst_cost)>=-.001 && use_heur>0.5 && 1==1)
			%disp('doingCalc')
			red_cost_old=red_cost;
