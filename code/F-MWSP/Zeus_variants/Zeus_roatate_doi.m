function [H,T,C]=Zeus_rotate_doi(G)
...
%		
%		rotation_list:  
%			index:  r,i:  holds the detection correpsonding to the ith index

%			size Nr,Nd
%			
%		inv_roation_list:
%			[~,inv_rotation_list]=sort(rotation_list);
%			index:  r,d.  holds the index of the ith detection
%			Nr,Nd
%		Q
%			any info relavent to the term
%-----------------------------------------
...
DOI.max_val=[];
DOI.max_trip=[];
DOI.max_val_list=zeros(G.B.Nd,G.opt.num_rotations_doi);

DOI.rotation_list=[];
DOI.inv_rotation_list=[];

for(r=1:G.opt.num_rotations_doi)

        DOI.rotation_list=[DOI.rotation_list;randperm(G.B.Nd)];
	[~,inv_ord]=sort(DOI.rotation_list(end,:));
	DOI.inv_rotation_list=[DOI.inv_rotation_list;inv_ord];
...
if(isfield(G,'T'))
	...
	DOI=update_DOI_rotate(G,C,T,inds_new,DOI);
...
while(true) 
	...
	if(  residual>-G.opt.epsilon && G.opt.max_trip_add>0.5  &&  (sum((gamma.*(1-gamma)>.01))>0.5) && pricing_option>0.5  )
		...
		DOI=update_DOI_rotate(G,C,T,[1:numel(gamma)],DOI);
	end
	t4=toc(t4);
