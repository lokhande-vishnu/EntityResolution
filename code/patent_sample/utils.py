...
def readData(input_file, golden_file, set_delim='**'):
    ...
    with open(golden_file) as f:
        reader = csv.DictReader(f)
        for idx, row in enumerate(reader):
            if int(row['person_id']) in data_d:
                label_d[int(row['person_id'])] = int(row['leuven_id'])
...
def readGoldData(input_file, golden_file, set_delim='**'):            
    ...
    with open(golden_file) as f:
        reader = csv.DictReader(f)
        for idx, row in enumerate(reader):
            label_d[int(row['person_id'])] = int(row['True Id'])
            data_d[int(row['person_id'])] = True

    with open(input_file) as f:
        reader = csv.DictReader(f)
        for idx, row in enumerate(reader):
            if int(row['person_id']) not in data_d:
                continue
            row = dict((k, v.lower()) for k, v in row.items())
            if row['Lat'] == row['Lng'] == '0.0' :
                row['LatLong'] = None
...
def makeTrainingPairs(data_d, label_d, sample_ratio=1.0, seed = 42):
    ...
    distinct_size = int(0.001 *  len(data_d) * len(data_d))
