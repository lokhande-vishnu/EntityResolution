...
def randomPairs(n_records, sample_size, seed = None):
    """
    ...
    if sample_size >= n:
        ...
        try:
            random.seed(seed)
            random_sample = random.sample(range(n), sample_size)
            random_pairs = numpy.array(random_sample,
                                       dtype='uint')
        except OverflowError:
            return randomPairsWithReplacement(n_records, sample_size, seed)
...
def randomPairsWithReplacement(n_records, sample_size, seed = None):
