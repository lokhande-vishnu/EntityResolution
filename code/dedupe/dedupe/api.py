...
class Matching(object):
    ...
    def matchBlocks(self, blocks, threshold=.5, *args, **kwargs):
        ...
        print('LOOK: Need to separate things at this point!!')
    ...
    def scoreBlocks(self, blocks, *args, **kwargs):
        """
        New code section
        """
        ...
        scores = core.scoreDuplicates(candidate_records,
                                       self.data_model,
                                       self.classifier,
                                       self.num_cores,
                                       threshold=0)
    ...
    def clusterScores(self, scores, threshold=.5, *args, **kwargs):
        """
        New code section
        """
        logger.debug("clustering...")

        for cluster in self._cluster(scores, threshold, *args, **kwargs):
            yield cluster

        try:
            score_file = scores.filename
            del scores
...
class DedupeMatching(Matching):
    ...
    def blockData(self, data_d):
        ...
        data_d_items = viewitems(data_d)
        block_items = self.blocker(data_d_items)
        block_groups = itertools.groupby(block_items,
                                         lambda x: x[1])
...
class StaticDedupe(DedupeMatching, StaticMatching):
    ...
    """
    print('USING LOCAL STATICDEDUPE!')
